...
class ClickAction {
	...
	public function toJson()
	{
		...
		$ret['package_name'] = array('packageDownloadUrl'=>$this->m_packageDownloadUrl, 
									 'confirm'=>$this->m_confirmOnPackageDownloadUrl,
									 'packageName'=>$this->m_packageName
								  );
	...
	public function isValid()
	{
		...
		if ($this->m_actionType<self::TYPE_ACTIVITY || $this->m_actionType>self::TYPE_PACKAGE)
			return false;
		...
		if($this->m_actionType == self::TYPE_PACKAGE)
		{
			if (is_string($this->m_packageDownloadUrl) && is_string($this->m_packageName) &&
			is_int($this->m_confirmOnPackageDownloadUrl) &&
			$this->m_confirmOnPackageDownloadUrl>=0 && $this->m_confirmOnPackageDownloadUrl<=1
			) {
				return true;
			}
			return false;
